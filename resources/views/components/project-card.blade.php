...
<div class="bg-white rounded-xl shadow-sm border transition-all duration-150
    ...
    @if($isEditing)
        {{-- EDIT MODE: Display form fields instead of static text --}}
        <div class="p-5 space-y-3">
            ...
            <textarea wire:model.defer="editDescription" rows="2"
                class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-400 resize-none">
            </textarea>

            <div class="flex gap-2">
                <button wire:click="updateProject" class="bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold px-4 py-2 rounded-lg">
                    Save Changes
                </button>
                <button wire:click="cancelEditing" class="bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs font-semibold px-4 py-2 rounded-lg">
                    Cancel
                </button>
            </div>
        </div>
    ...
    @else
        {{-- View Mode: Display project details and progress --}}
        <div class="p-5">
            <div class="flex items-start justify-between gap-4">
                ...
                <button wire:click="selectProject({{ $project->id }})" class="flex-1 text-left group">
                    ...
                    {{-- Progress Indicators: Shows completed tasks vs total tasks --}}
                ...
                {{-- Control Buttons: Toggle Tasks, Edit, and Delete --}}
                <div class="flex items-center gap-1 shrink-0">
                    ...
                    <button wire:click="deleteProject({{ $project->id }})"
                        wire:confirm="Are you sure you want to delete '{{ $project->name }}'?"
                        title="Delete"
        ...
        {{-- Slot for the TaskManager component when project is selected --}}
        {{ $slot }}
    @endif
</div>
